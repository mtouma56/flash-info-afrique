...
if (process.env.NODE_ENV === "production" && CRON_SECRET === DEFAULT_CRON_SECRET) {
  logger.warn("⚠️  CRITICAL: CRON_SECRET is using default value in production! Cron jobs will fail. Please set CRON_SECRET in environment variables.");
}
...
async function startServer() {
  ...
  app.get("/api/scrape-rss", async (req, res) => {
    ...
    try {
      ...
      logger.error("RSS scraping error (GET)", undefined, error);
      return res.status(500).json({ 
        error: "RSS scraping failed",
        message: error instanceof Error ? error.message : "Unknown error",
      });
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  app.get("*", (req, res, next) => {
    ...
    // Don't serve index.html for sitemap, rss.xml and robots.txt (already handled above)
    if (req.path === "/sitemap.xml" || req.path === "/news-sitemap.xml" || req.path === "/robots.txt" || req.path === "/rss.xml") {
      return next();
    }
